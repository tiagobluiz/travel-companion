...
class RestoreTripService(
    ...
    fun execute(tripId: TripId, userId: UserId): AccessResult<com.travelcompanion.domain.trip.Trip> {
        ...
        if (trip.status == TripStatus.ACTIVE) return AccessResult.Success(trip)
        return AccessResult.Success(tripRepository.save(trip.copy(status = TripStatus.ACTIVE)))
    }
}
